...
def get_user_info():
	...
	user.is_principal = "Principal" in user.roles
	user.is_student = not user.is_instructor and not user.is_moderator and not user.is_evaluator and not user.is_principal
...
def is_published_course(course: str) -> bool:
	return frappe.db.get_value("LMS Course", course, "published") == 1

@frappe.whitelist()
def process_approval(doctype: str, docname: str, action: str, comments: str = ""):
	...
	# Automatically publish if approved
	if action == "Approve" and doc.workflow_state == "Approved":
		apply_workflow(doc, "Publish")
	
	# If rejected, log a comment on the document
	if action == "Reject" and comments:
		frappe.get_doc({
			"doctype": "Comment",
...
@frappe.whitelist()
def get_rejection_reason(course: str):
	if not frappe.has_permission("LMS Course", "read", course):
		frappe.throw(_("Not permitted"), frappe.PermissionError)
	return frappe.get_all(
		"Comment",
		filters={"reference_doctype": "LMS Course", "reference_name": course, "comment_type": "Workflow"},
