...
from frappe.utils import add_days, cint, format_datetime, get_time, nowdate, getdate
...
class LMSBatch(Document):
	...
	def validate_published(self):
		...
		if self.is_new() and cint(self.published) == 1:
			published_changed = True
		elif not self.is_new() and cint(self.published) != cint(self.get_doc_before_save().get("published")):
			published_changed = True

		if published_changed:
			if "Principal" not in frappe.get_roles() and "System Manager" not in frappe.get_roles():
				frappe.throw(_("Only a Principal can publish or unpublish a batch."))
	...
	def calculate_expiry_date(self):
		# If batch does not have validity, try to inherit from first course
		if not self.validity_days and self.courses:
			course_doc = frappe.get_doc("LMS Course", self.courses[0].course)
			if course_doc.validity_days:
				self.validity_days = course_doc.validity_days

		if self.start_date and self.validity_days:
			self.expiry_date = add_days(self.start_date, cint(self.validity_days))
		else:
			self.expiry_date = None

		# Validate against course expiry dates
		if self.expiry_date and self.courses:
			for row in self.courses:
				course_expiry = frappe.db.get_value("LMS Course", row.course, "expiry_date")
				if course_expiry and getdate(self.expiry_date) > getdate(course_expiry):
					frappe.throw(_("Batch expiry date cannot extend beyond Course ({0}) expiry date: {1}").format(row.course, course_expiry))
