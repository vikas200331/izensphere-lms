...
def update_course_filters(filters: dict) -> tuple:
	...
	if user != "Guest":
		...
		if "System Manager" in roles or "Principal" in roles or "Course Creator" in roles or "Moderator" in roles:
			is_privileged = True

	# Enforce published rule if not filtering by created courses (which instructors see)
...
def get_course_details(course: str):
	...
	course_details.transitions = get_workflow_transitions(course_details.name, "LMS Course")
...
def update_batch_filters(filters: dict) -> None:
	...
	if user != "Guest":
		roles = frappe.get_roles(user)
		if "System Manager" in roles or "Principal" in roles or "Moderator" in roles or "Batch Evaluator" in roles:
			is_privileged = True
...
def get_workflow_transitions(docname, doctype):
	try:
